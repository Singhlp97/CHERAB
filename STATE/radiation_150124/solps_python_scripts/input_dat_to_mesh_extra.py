...
def input_dat_to_mesh_extra(where = ".", reorder = True, scale = 1E+00):

	# Author: Matteo Moscheni
	# E-mail: matteo.moscheni@tokamakenergy.co.uk
	# February 2022

	if isinstance(where, list) is False: where = [where]

	for here in where:

		try:    file = open(os.path.join(here, 'input.dat'), 'r')
		except: file = open(os.path.join(here, 'input.eir'), 'r')
		...
		mesh_extra = np.array(mesh_extra) * 1E-02 * scale
		mesh_extra = np.vstack((mesh_extra, mesh_extra[0,:]))

		file = open(os.path.join(here, 'mesh.extra'), 'w+')
